...
public class FileUpload {
    public static String[] Upload(FastDFSFile fastFile,String fileExt) throws Exception {
        ...
        metaList[0] = new NameValuePair("fileName", fastFile.getName());
        metaList[1] = new NameValuePair("fileExtName", fileExt);
        metaList[2] = new NameValuePair("fileLength", String.valueOf(fastFile.getSize()));
        //8、进行文件上传操作
        String[] uploadFile = storageClient.upload_file(fastFile.getContent(), fileExt, metaList);
        trackerServer.close();
        return uploadFile;
    }
}
