	...
	function toggleCart(bool) {
		...
		if( cartIsOpen ) {
			...
			setTimeout(function(){
				...
				if( Number(cartCount.find('li').eq(0).text()) == 0) cartWrapper.addClass('count');
			}, 500);
		} else {
...
/*
	...
	function detailBuy(id) {
		var sign = $('#sign').val();
		var rk = $('#rk').val();
		var t_uniq = localData.get();
			//selectGuigeSize();
		var data = "p_id="+id+"&t_uniq="+t_uniq;
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "/ajax.php?action=def_shopping&exec=add&sign="+sign+"&rk="+rk+"",
			//data: detail_cart.serialize(),
			data:data,
